...
# ---------------------------------------------
# Fonction : Installer un paquet si manquant
# ---------------------------------------------
install_pkg() {
        ...
        if [ -f /etc/os-release ]; then
            ...
            case "$ID" in
                ...
                opensuse*|sles)
                    ...
                    ;;
                *)
                    echo "× Impossible d’installer automatiquement $1"
                    exit 1
                    ;;
            esac
        fi
...
# ---------------------------------------------
# PARTIE 1 — PRÉPARATION MACHINE SUPERVISEUR
# ---------------------------------------------
# ========================================
#  Étape 1 : Configuration locale
# ========================================
echo "=== Étape 1 : Installation des dépendances locales ==="
install_pkg git
install_pkg python3
...
# Demande d'information sur le serveur à superviser
echo ""
echo "==== Veuiller fournir les informations suivants : ===="
read -rp "» Entrez le nom d'utilisateur du serveur à superviser : " REMOTE_USER
read -rp "» Entrez l'adresse IP du serveur à superviser : " REMOTE_IP
...
# ============================================
#  Étape 2 : Création de l'utilisateur apmf
# ============================================
echo "==== Étape 2 : Création de l'utilisateur de supervision ===="
if ! id "$MONITORING_USER" >/dev/null 2>&1; then
    sudo adduser --disabled-password --gecos "APMF Monitoring User" "$MONITORING_USER"
    echo "»» Utilisateur créer avec succès : ${MONITORING_USER}"
fi
echo ""

# ===========================================================
#  Étape 3 : Génération / vérification de la clé SSH locale
# ===========================================================
echo "==== Étape 3 : Génération/Vérification du clé SSH locale ===="
...
# --------------------------------------------------
# PARTIE 2 — CONFIGURATION DU SERVEUR À SUPERVISER
# --------------------------------------------------
# ==================================================
#  Étape 4 : Configuration du serveur à superviser
# ==================================================
echo "==== Étape 4 : Configuration du serveur à superviser ===="
...
# Test de connexion
echo "»»» Test de connexion avec la clé SSH «««"
...
# ---------------------------------------------------
# PARTIE 3 — INSTALLER / METTRE À JOUR LE COLLECTOR
# ---------------------------------------------------
# ======================================
# Étape 5 — Installation du Collector
# ======================================
echo "==== Étape 5 : Installation du Collector ===="

if [ -d "$COLLECTOR_PATH/.git" ]; then
    echo "» Mise à jour du repo collector..."
    sudo -u "$MONITORING_USER" git -C "$COLLECTOR_PATH" pull
else
    echo "» Suppression du répertoire existant..."
    sudo rm -rf "$COLLECTOR_PATH"
    echo "» Clonage du repo collector..."
    sudo -u "$MONITORING_USER" git clone "$REPO_URL" "$COLLECTOR_PATH"
fi

# Installer dépendances python
sudo -u "$MONITORING_USER" bash -c "
    python3 -m venv '$COLLECTOR_PATH/venv' || true &&
    source $COLLECTOR_PATH/venv/bin/activate &&
    pip install -r $COLLECTOR_PATH/requirements.txt
"

# Copier la clé SSH pour l'utilisateur apmf
echo "» Copie de la clé SSH pour l'utilisateur apmf..."
sudo mkdir -p /home/apmf/.ssh
sudo cp "$SSH_KEY_PATH" "$SSH_KEY_PATH.pub" /home/apmf/.ssh/
sudo chown -R apmf:apmf /home/apmf/.ssh
sudo chmod 600 /home/apmf/.ssh/apmf_key
echo "»» Clé SSH copiée dans /home/apmf/.ssh/"

# ============================================
# Étape 6 — Création du service systemd
# ============================================
echo "=== Étape 6 : Service systemd ==="
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"
sudo tee "$SERVICE_FILE" >/dev/null <<EOF
[Unit]
Description=APMF Monitoring Collector
...
User=apmf
Group=apmf
WorkingDirectory=$COLLECTOR_PATH
Environment="PATH=$COLLECTOR_PATH/venv/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
Environment="HOME=/home/apmf"
Environment="SSH_KEY_PATH=/home/apmf/.ssh/apmf_key"
ExecStart=$COLLECTOR_PATH/venv/bin/python $COLLECTOR_PATH/collector.py
Restart=always
RestartSec=10
